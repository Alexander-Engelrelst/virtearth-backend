...
public sealed class UpdateFoundMazeArtifacts(ILogger<UpdateFoundMazeArtifacts> logger) : IUseCase<UpdateFoundMazeArtifactsInput, MazeGame?>
{
    private readonly ILogger<UpdateFoundMazeArtifacts> _logger = logger;
    public MazeGame? Execute(UpdateFoundMazeArtifactsInput input)
    {
        ...
        return game.UpdateUserFoundArtifacts(input.ArtifactId, input.XCord,  input.YCord, input.Angle);
    }
}